...
export function AuthProvider({ children }) {
  ...
  // (reservado) sin función auxiliar de respuesta para cumplir lint

  // Login usando Fetch (envuelto en useCallback)
  const login = useCallback(async ({ email, password }) => {
    ...
    // Intentamos recuperar el perfil del usuario con /auth/me
    let newUser = null
    try {
      const meRes = await fetch(`${AUTH_BASE}/auth/me`, { headers: { Authorization: `Bearer ${newToken}` } })
      if (meRes.ok) {
        newUser = await meRes.json()
      }
    } catch (err) {
      console.warn('Fallo /auth/me:', err)
    }
    // Fallback si /auth/me no devuelve usuario
    if (!newUser) newUser = data?.user || data?.profile || { name: data?.name || email }
    setUser(newUser) // Guardamos usuario
    // Retornamos resultado
    return { token: newToken, user: newUser } // Devolvemos datos
  }, []) // Sin dependencias porque setToken y setUser son estables

  // Logout usando Fetch (envuelto en useCallback)
  const logout = useCallback(async () => {
    ...
    try {
      ...
      console.warn('Fallo /auth/logout:', err)
    }
  ...
  }, [token]) // Dependencia: token actual

  // Renovación del token usando Fetch (envuelto en useCallback)
  ...
  useEffect(() => {
    ...
    const id = setTimeout(async () => {
      ...
      if (ok) {
        try {
          await refreshToken() // Renovamos token
        } catch (err) {
  ...
  const value = useMemo(() => ({
    ...
    login, // Función de login con Fetch (useCallback)
    logout, // Función de logout con Fetch (useCallback)
    refreshToken, // Función para renovar el token (useCallback)
  }), [token, user, expiresAt, login, logout, refreshToken])
